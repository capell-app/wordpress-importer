...
1. **Make the Migration Assistant registry path-aware for WXR selection** — why: `WxrReader::supports()` can stream-sniff full readable XML paths and refuse generic XML, but MA's current `ImportSourceRegistry` still passes only `pathinfo(..., PATHINFO_EXTENSION)` to reader contracts. Until MA can pass a path or source descriptor, this reader must still claim extension-only `xml` to preserve the existing WXR upload path — `src/Services/WxrReader.php`, migration-assistant registry — M.
